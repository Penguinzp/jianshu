<template>
  <!--<b-img clss="img" src="../../static/img/four.png"/>-->
  <div>
    <b-card title=""
            img-alt="Img"
            img-top>
      <div class="clearfix">
        <b-row>
          <b-col cols="8">
            <a class="title" target="_blank" href="/h">海底捞董事长：我做了那么多亲情化举动，却“败给”一个吧台小姑娘</a>
            <p class="card-text article">
              01 刚当程序员时，我是属于那种勤勤恳恳类型的员工，工作态度用认真来形容不为过，
              每天我几乎是团队里最早到公司，又最晚下班的一个。而组员张工一般情...
            </p>
            <div class="meta">
              <a class="card-text" target="_blank" style="color: gray ;font-size: 16px">暮时说势</a>
              <a target="_blank">
                <i class="card-text">
                  <b-img clss="img"
                         style="opacity:0.5;
                         filter:alpha(opacity=50);"
                         src="../../static/img/remake.png"/>
                  6</i>
              </a> <span style="padding-left: 8px"><i class="card-text">
              <b-img class="img" src="../../static/img/like.png"/>9</i> </span>
            </div>
            <!--<div slot="footer">-->
              <!--<small class="text-muted">Last updated 3 mins ago</small>-->
            <!--</div>-->
          </b-col>
          <b-col cols="4">
            <b-img style = "width: 160px; height: 120px;" right src="//upload-images.jianshu.io/upload_images/5049362-40b1a106860650f7.jpg?imageMogr2/auto-orient/strip|imageView2/1/w/300/h/240" alt="right image" />
          </b-col>
        </b-row>
      </div>
    </b-card>
    <b-card title=""
            img-alt="Img"
            img-top>
      <div class="clearfix">
        <b-row>
          <b-col cols="8">
            <a class="title" target="_blank" href="/h">海底捞董事长：我做了那么多亲情化举动，却“败给”一个吧台小姑娘</a>
            <p class="card-text article">
              01 刚当程序员时，我是属于那种勤勤恳恳类型的员工，工作态度用认真来形容不为过，
              每天我几乎是团队里最早到公司，又最晚下班的一个。而组员张工一般情...
            </p>
            <div class="meta">
              <a class="card-text" target="_blank" style="color: gray ;font-size: 16px">暮时说势</a>
              <a target="_blank">
                <i class="card-text">
                  <b-img clss="img"
                         style="opacity:0.5;
                         filter:alpha(opacity=50);"
                         src="../../static/img/remake.png"/>
                  6</i>
              </a> <span style="padding-left: 8px"><i class="card-text">
              <b-img class="img" src="../../static/img/like.png"/>9</i> </span>
            </div>
            <!--<div slot="footer">-->
            <!--<small class="text-muted">Last updated 3 mins ago</small>-->
            <!--</div>-->
          </b-col>
          <b-col cols="4">
            <b-img style = "width: 160px; height: 120px;" right src="//upload-images.jianshu.io/upload_images/5049362-40b1a106860650f7.jpg?imageMogr2/auto-orient/strip|imageView2/1/w/300/h/240" alt="right image" />
          </b-col>
        </b-row>
      </div>
    </b-card>
    <b-card title=""
            img-alt="Img"
            img-top>
      <div class="clearfix">
        <b-row>
          <b-col cols="8">
            <a class="title" target="_blank" href="/h">海底捞董事长：我做了那么多亲情化举动，却“败给”一个吧台小姑娘</a>
            <p class="card-text article">
              01 刚当程序员时，我是属于那种勤勤恳恳类型的员工，工作态度用认真来形容不为过，
              每天我几乎是团队里最早到公司，又最晚下班的一个。而组员张工一般情...
            </p>
            <div class="meta">
              <a class="card-text" target="_blank" style="color: gray ;font-size: 16px">暮时说势</a>
              <a target="_blank">
                <i class="card-text">
                  <b-img clss="img"
                         style="opacity:0.5;
                         filter:alpha(opacity=50);"
                         src="../../static/img/remake.png"/>
                  6</i>
              </a> <span style="padding-left: 8px"><i class="card-text">
              <b-img class="img" src="../../static/img/like.png"/>9</i> </span>
            </div>
            <!--<div slot="footer">-->
            <!--<small class="text-muted">Last updated 3 mins ago</small>-->
            <!--</div>-->
          </b-col>
          <b-col cols="4">
            <b-img style = "width: 160px; height: 120px;" right src="//upload-images.jianshu.io/upload_images/5049362-40b1a106860650f7.jpg?imageMogr2/auto-orient/strip|imageView2/1/w/300/h/240" alt="right image" />
          </b-col>
        </b-row>
      </div>
    </b-card>
    <b-card title=""
            img-alt="Img"
            img-top>
      <div class="clearfix">
        <b-row>
          <b-col cols="8">
            <a class="title" target="_blank" href="/h">海底捞董事长：我做了那么多亲情化举动，却“败给”一个吧台小姑娘</a>
            <p class="card-text article">
              01 刚当程序员时，我是属于那种勤勤恳恳类型的员工，工作态度用认真来形容不为过，
              每天我几乎是团队里最早到公司，又最晚下班的一个。而组员张工一般情...
            </p>
            <div class="meta">
              <a class="card-text" target="_blank" style="color: gray ;font-size: 16px">暮时说势</a>
              <a target="_blank">
                <i class="card-text">
                  <b-img clss="img"
                         style="opacity:0.5;
                         filter:alpha(opacity=50);"
                         src="../../static/img/remake.png"/>
                  6</i>
              </a> <span style="padding-left: 8px"><i class="card-text">
              <b-img class="img" src="../../static/img/like.png"/>9</i> </span>
            </div>
            <!--<div slot="footer">-->
            <!--<small class="text-muted">Last updated 3 mins ago</small>-->
            <!--</div>-->
          </b-col>
          <b-col cols="4">
            <b-img style = "width: 160px; height: 120px;" right src="//upload-images.jianshu.io/upload_images/5049362-40b1a106860650f7.jpg?imageMogr2/auto-orient/strip|imageView2/1/w/300/h/240" alt="right image" />
          </b-col>
        </b-row>
      </div>
    </b-card>
    <b-card title=""
            img-alt="Img"
            img-top>
      <div class="clearfix">
        <b-row>
          <b-col cols="8">
            <a class="title" target="_blank" href="/h">海底捞董事长：我做了那么多亲情化举动，却“败给”一个吧台小姑娘</a>
            <p class="card-text article">
              01 刚当程序员时，我是属于那种勤勤恳恳类型的员工，工作态度用认真来形容不为过，
              每天我几乎是团队里最早到公司，又最晚下班的一个。而组员张工一般情...
            </p>
            <div class="meta">
              <a class="card-text" target="_blank" style="color: gray ;font-size: 16px">暮时说势</a>
              <a target="_blank">
                <i class="card-text">
                  <b-img clss="img"
                         style="opacity:0.5;
                         filter:alpha(opacity=50);"
                         src="../../static/img/remake.png"/>
                  6</i>
              </a> <span style="padding-left: 8px"><i class="card-text">
              <b-img class="img" src="../../static/img/like.png"/>9</i> </span>
            </div>
            <!--<div slot="footer">-->
            <!--<small class="text-muted">Last updated 3 mins ago</small>-->
            <!--</div>-->
          </b-col>
          <b-col cols="4">
            <b-img style = "width: 160px; height: 120px;" right src="//upload-images.jianshu.io/upload_images/5049362-40b1a106860650f7.jpg?imageMogr2/auto-orient/strip|imageView2/1/w/300/h/240" alt="right image" />
          </b-col>
        </b-row>
      </div>
    </b-card>
    <b-card title=""
            img-alt="Img"
            img-top>
      <div class="clearfix">
        <b-row>
          <b-col cols="8">
            <a class="title" target="_blank" href="/h">强烈推荐几款有趣好用的手机APP，你值得拥有！</a>
            <p class="card-text article">
              To-Do ​To-do 是一个微软公司推出的智能待办事项App。与其他待办事项App 相比，用户可以区分不同的待办内容，比如生活，工作，旅游等...
            </p>
            <div class="meta">
              <a class="card-text" target="_blank" style="color: gray ;font-size: 16px">暮时说势</a>
              <a target="_blank">
                <i class="card-text">
                  <b-img clss="img"
                         style="opacity:0.5;
                         filter:alpha(opacity=50);"
                         src="../../static/img/remake.png"/>
                  0</i>
              </a> <span style="padding-left: 8px"><i class="card-text">
              <b-img class="img" src="../../static/img/like.png"/>0</i> </span>
            </div>
            <!--<div slot="footer">-->
            <!--<small class="text-muted">Last updated 3 mins ago</small>-->
            <!--</div>-->
          </b-col>
          <b-col cols="4">
            <b-img style = "width: 160px; height: 120px;" right src="//upload-images.jianshu.io/upload_images/7078796-a7d2cc302fa2a020.png?imageMogr2/auto-orient/strip|imageView2/1/w/300/h/240" />
          </b-col>
        </b-row>
      </div>
    </b-card>
    <b-card title=""
            img-alt="Img"
            img-top>
      <div class="clearfix">
        <b-row>
          <b-col cols="8">
            <a class="title" target="_blank" href="/h">海底捞董事长：我做了那么多亲情化举动，却“败给”一个吧台小姑娘</a>
            <p class="card-text article">
              01 刚当程序员时，我是属于那种勤勤恳恳类型的员工，工作态度用认真来形容不为过，
              每天我几乎是团队里最早到公司，又最晚下班的一个。而组员张工一般情...
            </p>
            <div class="meta">
              <a class="card-text" target="_blank" style="color: gray ;font-size: 16px">教程大咖</a>
              <a target="_blank">
                <i class="card-text">
                  <b-img clss="img"
                         style="opacity:0.5;
                         filter:alpha(opacity=50);"
                         src="../../static/img/remake.png"/>
                  6</i>
              </a> <span style="padding-left: 8px"><i class="card-text">
              <b-img class="img" src="../../static/img/like.png"/>9</i> </span>
            </div>
            <!--<div slot="footer">-->
            <!--<small class="text-muted">Last updated 3 mins ago</small>-->
            <!--</div>-->
          </b-col>
          <b-col cols="4">
            <b-img style = "width: 160px; height: 120px;" right src="//upload-images.jianshu.io/upload_images/5049362-40b1a106860650f7.jpg?imageMogr2/auto-orient/strip|imageView2/1/w/300/h/240" alt="right image" />
          </b-col>
        </b-row>
      </div>
    </b-card>
  </div>

</template>

<script>
    export default {
        name: "Article"
    }
</script>

<style scoped>
.title{
  color: black;
  font-weight: bold;
}.article{
  color: gray;
 }
</style>
